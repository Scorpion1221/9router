...
export function createCodexTierAudit({ provider, model, body, finalBody, status, reqTag, log }) {
  ...
  return {
    ...
    finish() {
      if (finished) return;
      finished = true;
      log.line(reqTag, "⚡", `CODEX_TIER client_model=${safeModel(body?.model)} upstream_model=${safeModel(model)} client_tier=${safeTier(body?.service_tier)} wire_tier=${safeTier(finalBody?.service_tier)} created_tier=${created} completed_tier=${completed} terminal=${terminal} http=${status}`);
    },
  };
}
