...
export function createSSEStream(options = {}) {
  ...
  const state = mode === STREAM_MODE.TRANSLATE
    ? { ...initState(sourceFormat), provider, toolNameMap, customToolNames: new Set(customToolNames || []), namespaceTools, model, sessionId: credentials?._clientSessionId || null,
    ...
    transform(chunk, controller) {
      ...
      for (const line of lines) {
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
          ...
          if (passthroughDone && streamDoneSent) continue;
      ...
      try {
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
          ...
          if (buffer && (!bufferedDone || !streamDoneSent)) {
            ...
            reqLogger?.appendConvertedChunk?.(output);
            controller.enqueue(sharedEncoder.encode(output));
            if (bufferedDone) streamDoneSent = true;
          }
