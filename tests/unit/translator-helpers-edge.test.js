...
describe("normalizeClaudePassthrough — haiku adaptive thinking (docs 11 §1)", () => {
  ...
  it.each(["system", "developer"])("folds mid-conversation %s instructions without changing the cached prefix", (role) => {
    const body = {
      system: [{ type: "text", text: "stable persona" }],
      messages: [
        { role: "user", content: "hi" },
        { role, content: "be brief" },
      ],
    };
    const originalMessage = body.messages[0];
    const out = normalizeClaudePassthrough(body);
    expect(out.system).toEqual([{ type: "text", text: "stable persona" }]);
    expect(out.messages).toEqual([{ role: "user", content: [
      { type: "text", text: "hi" }, { type: "text", text: "be brief" },
    ] }]);
    expect(originalMessage.content).toBe("hi");
  });
});
