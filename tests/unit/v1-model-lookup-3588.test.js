...
vi.mock("@/lib/modelInfo", () => ({
  resolveModelInfo: mocks.resolveModelInfo,
}));
...
describe("GET /v1/models/{id}", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.resolveModelInfo.mockReset();
  });
  ...
  it("preserves live metadata on capability-list routes after the catch-all migration", async () => {
    const model = { id: "embedding/test", object: "model", owned_by: "embedding" };
    mocks.buildModelsList.mockResolvedValue([model]);
    mocks.resolveModelInfo.mockResolvedValue({
      contextWindow: 8192,
      maxOutput: 1024,
      pricing: { input: 2, output: 8, cached: 0.5 },
    });

    const response = await GET(new Request("https://router.test/v1/models/embedding"), params(["embedding"]));

    expect(await response.json()).toEqual({ object: "list", data: [{
      ...model,
      context_length: 8192,
      max_completion_tokens: 1024,
      pricing: { prompt: "0.000002", completion: "0.000008", input_cache_read: "5e-7" },
    }] });
    expect(mocks.resolveModelInfo).toHaveBeenCalledWith(model.id);
  });
});
