...
const C2K = (body, credentials = null, model = "claude-sonnet-4.5") => {
  const out = translateRequest(FORMATS.CLAUDE, FORMATS.KIRO, model, body, true, credentials, "kiro");
  // Current Kiro surfaces reject this field; instructions travel in conversation text.
...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("keeps Claude system in the first user turn without a top-level systemPrompt", () => {
    const out = C2K({
  ...
  it("keeps the instruction-bearing first turn frozen across follow-up turns", () => {
    const credentials = { connectionId: "stable-instruction-account", rawHeaders: { "x-session-id": "stable-instruction-session" } };
    const first = C2K({
    ...
    expect(frozen).toContain("Current time");
